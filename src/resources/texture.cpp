...
namespace Resources {
    Texture::Texture(Rendering::GPU::ColorType color_type, const glm::vec2& size, const void* data) :
            ...
            size(size) {
        id = Rendering::GPU::gpu.createTexture(color_type, static_cast<glm::uvec2>(size), data);
    }
    ...
    Texture::~Texture() { Rendering::GPU::gpu.destroyTexture(id); }

    void Texture::set_size(const glm::vec2& _size) {
        if (_size == get_size()) return;
        Rendering::GPU::gpu.resizeTexture(shared_from_this(), static_cast<glm::uvec2>(_size));
        this->size = _size;
    }
}