...
namespace Platform::Game::Components {
    ...
    // TODO: there is an inherent problem here in that getRay will likely be called before
    //  the parameters have been calculated properly, and will cause the results of this
    //  function to lag one frame behind what's actually on the screen. One solution would
    //  be to have components know when their pose (eg. on_pose_changed)has been changed.
    //  In this case, we would listen for on_pose_changed, and then, if necessary, we would
    //  recalculate the parameters (by calling get_params) and then get the ray as normal.
    //  This would be problematic because it would probably add thousands of calls to
    //  components that have no business responding to pose changes. The other solution
    //  would be to always recalculate the matrices every frame. The calculations are
    //  negligible and would only be happening once per frame. Another option is to simply
